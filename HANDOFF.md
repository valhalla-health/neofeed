...
Still outstanding, and **not** blocking: `applyPatHeaderColumns()` has not been run — it only labels the `multiplesCount` header in `Patient_Registry` R1, and `registerPatient` widens the grid on demand, so saving works without it.
