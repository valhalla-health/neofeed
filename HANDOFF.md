...
`applyPatHeaderColumns()` was **run by Praew from the Apps Script editor on 2026-08-17**, immediately after this deploy — `Patient_Registry` R1 is now labelled `multiplesCount`. It was cosmetic either way (the column is read and written by index, and `registerPatient` widens the grid on demand), so no migration is outstanding on any tab.
