...
(async () => {
  ...
  await section('§6 UP-C1 · every printed figure is what 42ce553 printed', async () => {
    ...
    //
    // All six recaptured 2026-09-22, twice, and NO printed number moved:
    //  1. TPN team feedback + Praew's no-trailing-zero rule. Compared figure
    //     by figure with main (6ee2762): the two cells of the new "ZnSO₄
    //     (Additional to the above)" line, "—" with no zinc, and the TPN
    //     volume without its trailing zero (60.0 → 60, 137.0 → 137, …).
    //  2. The two-page form (Praew: the doctor's page as the KCMH paper form,
    //     one row per product; pharmacy's working on the back). The figures
    //     come in another order and the back repeats the mL pharmacy draws
    //     up, so they were compared as sets with main: every number main
    //     printed is still printed, and no number is new; only the section
    //     headings ("1. Na⁺" … "Bag make-up") stopped being <strong>.
    // Was: e7466628370cc834, f8a53682fe1effb3, d70798321bad13ef,
    // 5baf4705b9b7adbd, a53c3c41e71fcc2d, abaff1c44614ae64 (main).
    const ORDERS = {
