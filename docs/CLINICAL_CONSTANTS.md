...
|---|---|---|
| `2026-09-18.1` | 2026-09-18 | **No existing value changed.** Added `KCMH_STOCK.aminoplasmal15` (0.15 g/mL), which **no ward is offered today**: its label contraindicates it under 2 years, and `aaProductsFor` allows it only on a ward in `OLDER_CHILD_WARDS`, which is empty (Praew). Added `MEN_MAX_ML_KG` = 24 mL/kg/d, an alert threshold that moves no dose. On an existing ward the only printed change is logic, not a constant: a MEN feed no longer counts toward the printed totals (`CHANGELOG.md` 2026-09-18 (2)). |
