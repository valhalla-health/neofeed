...
**แพลตฟอร์ม:** React 18 (โฮสต์เอง) โดยซอร์ส `.jsx` ถูกคอมไพล์ล่วงหน้าด้วย `tools/build.mjs` และ commit ผลลัพธ์ไว้ใน git
(ตั้งแต่ 2026-09-17 แทน Babel ในเบราว์เซอร์); ส่วนหน้าเป็นไฟล์ static ที่ Cloudflare Workers และ GitHub Pages ให้บริการตรงจาก git;
...
- **ไม่มีการ build บนโฮสต์ และไฟล์ที่ให้บริการต้องตรงกับไฟล์ใน git ทุกไบต์ — เป็นคุณสมบัติด้านความปลอดภัย ไม่ใช่เพียงความสะดวก**
  ข้อจำกัดนี้แทนที่ข้อจำกัดเดิม “ไม่มีขั้นตอน build” อย่างตั้งใจเมื่อ 2026-09-17 การคอมไพล์ JSX ด้วย Babel ในเบราว์เซอร์
  ทำให้ต้องรอหน้าจอ login ราว 4.6 วินาทีทุกครั้งที่โหลด (35 วินาทีบนเครื่องที่ช้า) และบังคับให้ CSP ต้องยอมรับ
  `'unsafe-eval'`/`'unsafe-inline'` ปัจจุบันซอร์ส `.jsx` ถูกคอมไพล์ด้วย `tools/build.mjs` บนเครื่องนักพัฒนา แล้ว commit
  ผลลัพธ์ (`compiled/`, `vendor/`) คู่กับซอร์ส สิ่งที่ข้อจำกัดเดิมปกป้องยังคงอยู่ครบ: Cloudflare และ GitHub Pages ไม่รันขั้นตอน
  build ใดๆ ไฟล์ที่เบราว์เซอร์ในหอผู้ป่วยได้รับคือไฟล์ใน git และ CI จะคอมไพล์ใหม่จากซอร์สแล้วปฏิเสธ PR หากผลลัพธ์ต่างจากที่ commit
  ไว้แม้เพียงไบต์เดียว สิ่งที่ผู้ตรวจทานอ่านจึงยังเป็นสิ่งที่รันจริง ไฟล์ที่ไม่ใช่ความลับทุกไฟล์ใน repository นี้ถือว่าเปิดเผยต่อ
  สาธารณะโดยปริยาย (ผลลัพธ์ที่คอมไพล์แล้วยังมีข้อความทุกตัวของซอร์ส) ห้ามใส่ข้อมูลลับในซอร์สโค้ด ด้วยเหตุนี้
  `SPREADSHEET_ID`/`CLIENT_ID` จึงถูกเก็บไว้ใน Script Properties
...
- **มี HTML shell สองไฟล์ที่ซิงก์ด้วยมือ** การเปลี่ยนแปลง shell ทุกครั้งต้องแก้ไขทั้งสองไฟล์ ยกเว้นโทเค็น `?v=` ที่ build
  เขียนให้ทั้งสองไฟล์ ตั้งแต่ 2026-09-17 ความแตกต่างจะไม่เล็ดลอดไปโดยไม่มีสัญญาณเตือนอีก เพราะ build จะไม่ทำงานหากสองไฟล์ต่างกัน
