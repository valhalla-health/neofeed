...
import {CenterPointClient} from './client.mjs';import {buildTpn} from './tpn-snapshot.mjs';import {renderTpn} from './tpn-document.mjs';import {orderSetup} from './order-setup.mjs';
window.React=React;
...
// DOL is not typed: it is NeoFeed's D.dolAtDate for the TPN date, from the date of birth (order-setup.mjs), so
// the DOL CP saves and prints is the one every NeoFeed screen shows. It appears as soon as both dates are in.
const setupValues=f=>({bw:f.bw.value,ga:f.ga.value,dob:f.dob.value,order:f.order.value});
$('setup').addEventListener('input',()=>{const s=orderSetup(setupValues($('setup').elements),window.NEOFEED_DATA);$('dol-out').textContent=s.error?'':`DOL ${s.dol}`;});
$('setup').onsubmit=e=>{e.preventDefault();if(!link)return;const f=e.currentTarget.elements;const setup=orderSetup(setupValues(f),window.NEOFEED_DATA);if(setup.error){$('feedback').textContent=setup.error;return;}const metadata={bw:Number(f.bw.value),ga:Number(f.ga.value),dol:setup.dol,order:f.order.value,measured:f.measured.value,from:new Date(f.from.value+'+07:00').toISOString(),to:new Date(f.to.value+'+07:00').toISOString(),source:f.source.value};if(metadata.to<=metadata.from){$('feedback').textContent='เวลาสิ้นสุดต้องอยู่หลังเวลาเริ่ม';return;}markDirty();
 ...
 root??=createRoot($('calculator'));root.render(React.createElement(window.Calculator,{key:crypto.randomUUID(),patient:{sessionId:link.localId,...setup.patient},dol:setup.dol,logDate:metadata.order,centerPoint:bridge}));$('setup').hidden=true;
};
